...
frame_parser[C.FRAME_TYPE.REMOTE_COMMAND_RESPONSE] = function(frame, buffer) {
  //console.log("3.frame_parser " + buffer.length, buffer);
  frame.id = buffer.readUInt8(0);
  ...
  // Avoid error access beyond buffer length. Frame is shorter than expected. 0xf minimum frame length expected.
  if(buffer.length < 0xf-1){
    frame.commandStatus = undefined;
    frame.commandData = undefined;
    return;
  }
...
frame_parser[C.FRAME_TYPE.ROUTE_RECORD] = function(frame, buffer) {
  ...
  // WARNING!!!!!!  Don't know why after a ND broadcast, numberOfAddresses byte
  // from Route Record Indicator is lost. It appear to be a problem from the remote 
  // module when transmiting the frame, because length byte also is wrong.
  // Workaround: when frame are smaller than the expected, return.
  // Avoid error access beyond buffer length. Frame is shorter than expected. 0xd minimum frame length expected.
  if(buffer.length < 0xd-1){
    frame.numberOfAddresses = 0;
    frame.hopsAddresses = [];  
    return;
  }
  
  frame.numberOfAddresses = buffer.readUInt8(11);
  frame.hopsAddresses = [];
  for(var i=0; i<frame.numberOfAddresses; i++){
    // Store 16bit addresses in hopAddresses array.
    frame.hopsAddresses[i] = frame_parser.parseAddress(buffer, 12+2*i, 2);
  }
};
